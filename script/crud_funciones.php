...
if($condicion=='table1'){
	...
	if($sede!=''){
		$sede = ' and id_roles = "'.$sede.'"';
	}

	$limit = $consultasporpagina;
	$offset = ($pagina - 1) * $consultasporpagina;

	$sql1 = "SELECT * FROM funciones WHERE estatus != 0 ".$filtrado." ".$sede." ";
	$sql2 = "SELECT * FROM funciones WHERE estatus != 0 ".$filtrado." ".$sede." ORDER BY id ASC LIMIT ".$limit." OFFSET ".$offset."";
	...
	if($conteo1>=1){
		while($row2 = mysqli_fetch_array($proceso2)) {
			...
			$sql3 = "SELECT * FROM roles WHERE id = ".$id_roles;
			$proceso3 = mysqli_query($conexion,$sql3);
			while($row3 = mysqli_fetch_array($proceso3)) {
				$roles_nombre = $row3["nombre"];
			}
			...
			$html .= '
		                <tr id="tr_'.$row2["id"].'">
		                    ...
		                    <td style="text-align:center;">'.$roles_nombre.'</td>
